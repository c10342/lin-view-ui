...
function writePakcageTpl(){
    ...
    const tplStr = fs.readFileSync(path.resolve(__dirname,'./template/package.tpl'), 'utf-8')
...
function writeIndexTpl(){
    ...
    const tplStr = fs.readFileSync(path.resolve(__dirname,'./template/index.tpl'), 'utf-8')
...
function writeVueTpl(){
    ...
    const tplStr = fs.readFileSync(path.resolve(__dirname,'./template/vue.tpl'), 'utf-8')
    const vueStr = handlebars.compile(tplStr)(parmas)
    fs.writeFileSync(path.resolve(compomentSrcPath,`${componentName}.vue`),vueStr)
}

function writeScssTpl(){
  const tplStr = fs.readFileSync(path.resolve(__dirname,'./template/scss.tpl'), 'utf-8')
  fs.writeFileSync(path.resolve(themeChalkSrc,`${componentName}.scss`),tplStr)
  fs.appendFileSync(path.resolve(themeChalkSrc,`index.scss`),`\n@import './${componentName}.scss';`)
}
